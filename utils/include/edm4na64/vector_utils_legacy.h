...
#ifndef EDM4NA64_UTILS_VECTOR_LEGACY_HH
#define EDM4NA64_UTILS_VECTOR_LEGACY_HH
...
#if __cpp_concepts
#include <edm4na64/vector_utils.h>
#else
#include <cmath>
...
V sphericalToVector(const double r, const double theta, const double phi) {
  using FloatType = decltype(edm4na64::vector_x(V()));
...
template <class V> double anglePolar(const V& v) {
  return std::atan2(std::hypot(edm4na64::vector_x(v), edm4na64::vector_y(v)),
                    edm4na64::vector_z(v));
}
template <class V> double angleAzimuthal(const V& v) {
  return std::atan2(edm4na64::vector_y(v), edm4na64::vector_x(v));
}
template <class V> double eta(const V& v) { return angleToEta(anglePolar(v)); }
template <class V> double magnitude(const V& v) {
  return std::hypot(edm4na64::vector_x(v), edm4na64::vector_y(v), edm4na64::vector_z(v));
}
template <class V> double magnitudeTransverse(const V& v) {
  return std::hypot(edm4na64::vector_x(v), edm4na64::vector_y(v));
}
template <class V> double magnitudeLongitudinal(const V& v) {
  return edm4na64::vector_z(v);
}
...
template <class V> V vectorTransverse(const V& v) {
  return {edm4na64::vector_x(v), edm4na64::vector_y(v), 0};
}
template <class V> V vectorLongitudinal(const V& v) {
  return {0, 0, edm4na64::vector_z(v)};
}
...
inline edm4hep::Vector2f operator+(const edm4hep::Vector2f& v1,
  ...
  using ValueType = decltype(edm4na64::vector_x(edm4hep::Vector2f()));
  const ValueType x = edm4na64::vector_x(v1) + edm4na64::vector_x(v2);
  const ValueType y = edm4na64::vector_y(v1) + edm4na64::vector_y(v2);
  return {x, y};
}
inline edm4hep::Vector3f operator+(const edm4hep::Vector3f& v1,
                                   const edm4hep::Vector3f& v2) {
  using ValueType = decltype(edm4na64::vector_x(edm4hep::Vector3f()));
  const ValueType x = edm4na64::vector_x(v1) + edm4na64::vector_x(v2);
  const ValueType y = edm4na64::vector_y(v1) + edm4na64::vector_y(v2);
  const ValueType z = edm4na64::vector_z(v1) + edm4na64::vector_z(v2);
  return {x, y, z};
}
inline double operator*(const edm4hep::Vector2f& v1, const edm4hep::Vector2f& v2) {
  return edm4na64::vector_x(v1) * edm4na64::vector_x(v2) +
         edm4na64::vector_y(v1) * edm4na64::vector_y(v2);
}
inline double operator*(const edm4hep::Vector3f& v1, const edm4hep::Vector3f& v2) {
  return edm4na64::vector_x(v1) * edm4na64::vector_x(v2) +
         edm4na64::vector_y(v1) * edm4na64::vector_y(v2) +
         edm4na64::vector_z(v1) * edm4na64::vector_z(v2);
}
inline edm4hep::Vector2f operator*(const double d, const edm4hep::Vector2f& v) {
  using ValueType = decltype(edm4na64::vector_x(edm4hep::Vector2f()));
  const ValueType x = d * edm4na64::vector_x(v);
  const ValueType y = d * edm4na64::vector_y(v);
  return {x, y};
}
inline edm4hep::Vector3f operator*(const double d, const edm4hep::Vector3f& v) {
  ...
  const ValueType x = d * edm4na64::vector_x(v);
  const ValueType y = d * edm4na64::vector_y(v);
  const ValueType z = d * edm4na64::vector_z(v);
  return {x, y, z};
}
